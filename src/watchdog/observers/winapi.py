...
import contextlib
import ctypes
from ctypes.wintypes import BOOL, DWORD, HANDLE, LPCWSTR, LPVOID, LPWSTR, INT, LARGE_INTEGER, WCHAR
...
ReadDirectoryChangesExW.argtypes = (
    ...
    ctypes.POINTER(OVERLAPPED),  # lpOverlapped
    LPVOID,  # FileIOCompletionRoutine # lpCompletionRoutine
    INT,  # READ_DIRECTORY_NOTIFY_INFORMATION_CLASS
)
...
class FILE_NOTIFY_EXTENDED_INFORMATION(ctypes.Structure):
    _fields_ = (
        ...
        ("FileAttributes", DWORD),  # actually a UNION
...
LPFNI = ctypes.POINTER(FILE_NOTIFY_EXTENDED_INFORMATION)
...
def _parse_event_buffer(read_buffer: bytes, n_bytes: int) -> list[tuple[int, str]]:
    ...
    while n_bytes > 0:
        ...
        ptr = ctypes.addressof(fni) + FILE_NOTIFY_EXTENDED_INFORMATION.FileName.offset
        filename = ctypes.string_at(ptr, fni.FileNameLength)

        is_dir = bool(fni.FileAttributes & FILE_ATTRIBUTE_DIRECTORY)
        results.append((fni.Action, filename.decode("utf-16"), is_dir))
...
#    GetFinalPathNameByHandleW(handle, buff, PATH_BUFFER_SIZE, VOLUME_NAME_NT)  # \Device\HarddiskVolume1\Users\fluxus\Desktop\watch\test test
#    print(buff.value, path)
...
#    event = FileNotifyInformation(0, FILE_ACTION_REMOVED_SELF, 2, b'.')

    event = FILE_NOTIFY_EXTENDED_INFORMATION(
        Action = FILE_ACTION_REMOVED_SELF,
        FileNameLength = 2,  #len(path),
        FileName = '.',  # path.value.encode("utf-8"),
